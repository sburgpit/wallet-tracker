...
export default buildConfig({
  ...
  cors: [process.env.PAYLOAD_PUBLIC_SERVER_DOMAIN || '', process.env.PAYLOAD_PUBLIC_SITE_URL || ''].filter(Boolean),
  csrf: [process.env.PAYLOAD_PUBLIC_SERVER_DOMAIN || '', process.env.PAYLOAD_PUBLIC_SITE_URL || ''].filter(Boolean),
  debug: true,
})
